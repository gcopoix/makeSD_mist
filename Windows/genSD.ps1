...
function c64_roms           { param ($1,$2,$3)
                              ...
                              download_url 'https://www.c64.com/games/download.php?id=315' "$3/zaxxon.zip" | Out-Null # zaxxon.zip
                              download_url 'https://www.c64.com/games/download.php?id=2073' "$3/super_zaxxon.zip" | Out-Null # super_zaxxon.zip
...
function neogeo_roms        { param ($1,$2,$3)
                              if ($SYSTEM -eq 'mist') {
                                ...
                                download_url 'https://downgit.github.io/#/home?url=https://github.com/mist-devel/mist-binaries/tree/master/cores/neogeo/bios' "$3/"
                                expand "$3/bios.zip" "$3/"
                                copy_mra_arcade_cores "$3/" '' "$2"
                                copy_mra_arcade_cores "$2/bios/cores/neogeo/bios" '' "$2"
                                Remove-Item "$3/bios/" -Recurse -Force
                              }
...
function tnzs_roms          { param ($1,$2,$3)
                              download_url 'https://downgit.github.io/#/home?url=https://github.com/jotego/jtbin/tree/master/mra' "$3/" | Out-Null
                              expand "$3/mra.zip" "$3/"
                              copy_mra_arcade_cores "$3/mra" '' "$2"
                            }
...
function copy_sidi_cores {
  ...
  # some some parameters to be distinguished between SiDi and SiDi128 handling
  $paramSet=@(
  ...
  foreach ($dir in (Get-ChildItem -Recurse $srcroot/Cores -Directory | Select-Object  -ExpandProperty FullName | Sort-Object | replace-slash)) {
    if ((($dir.Split('/'))[-1] -ne 'old') -and (($dir.Split('/'))[-1] -ne 'output_files')) {
      if (Test-Path "$dir/*.rbf") {
        ...
        foreach($item in $cores) {
          $dst = $item[0]
          $src = $item[2].replace('Arcade/',"Arcade/$($params[2])/") # SiDi Arcade folders are seperated between SiDi and SiDi128
          $hdl = $item[3]
          if ("$srcroot/Cores/$src" -eq $dir) {
            # support for optional testing of single specific core
            if (($null -ne $testcore) -and ($testcore -ne $dst)) { continue }
