...
# cache folder for repositories and MAME ROMs
...
function clone_or_update_git {
  ...
  if (-not (Test-Path -Path "$dstpath/.git" -pathtype Container)) {
    ...
    &"$TOOLS_ROOT/git/cmd/git.exe" clone $url $dstpath
  } else {
    ...
    if ($LASTEXITCODE -gt 0) {
      ...
      &"$TOOLS_ROOT/git/cmd/git.exe" -C "$dstpath/" pull
    } else {
  ...
  foreach ($f in &"$TOOLS_ROOT/git/cmd/git.exe" -C "$dstpath/" ls-tree -r --name-only HEAD) {
    ...
    $itm = Get-Item (Escape-Name "$dstpath/$f")
...
function coppy {
  ...
  if (Test-Path (Escape-Name $src)) {
    ...
    if ((Test-Path -Path (Escape-Name $src) -pathtype leaf) -and (Test-Path -Path $dst -pathtype leaf)) {
      $tms = (Get-Item -Path (Escape-Name $src) -Force).LastWriteTime
      if ((Get-Item -Path $dst -Force).LastWriteTime -ge $tms) {
        Write-Host "  `'$src`' skipped."
        return
      }
    }
...
function expand {
  ...
  if (Test-Path (Escape-Name $srcfile)) {
    # create destination folder if it doesn't exist
    ...
    switch -Wildcard ($srcfile) {
      '*.zip' { try { Expand-Archive -Path (Escape-Name $srcfile) -DestinationPath $dstpath -Force; Write-Host " done." } catch { Write-Host -ForegroundColor red " failed." } }
...
function download_url {
  ...
  if ((Test-Path $dst) -And ((Get-Item $dst).length -gt 0)) {
    ...
    try {
      ...
      $f = Get-Item $dst
      $resp = [System.Net.WebRequest]::Create((Escape-Name $url)).GetResponse()
...
function Escape-Name {
  param ( [string]$path ) # $1: string to be PowerShell-like escaped

  # escape square brackets with filenames - necessary for (some) PowerShell methods
...
function grep {
  ...
  $result = "$((Select-String -Pattern $pattern -Path (Escape-Name $f)).Matches)"
  return $result
}
...
function download_mame_roms {
  ...
  $romlookup=@(
   ...
   #( 'xevious.zip',        'htts://ia802803.us.archive.org/35/items/MAME216RomsOnlyMerged/xevious.zip' ), #can't download
    ...
    ( 'jtbeta.zip',         'http://ia804503.us.archive.org/1/items/jtkeybeta/beta.zip'                 )  #http://twitter.com/jtkeygetterscr1/status/1403441761721012224?s=20&t=xvNJtLeBsEOr5rsDHRMZyw
  )
...
function mra {
  ...
  $name = (Get-Item (Escape-Name $mrafile)).Basename
  ...
  if ($setname.length -gt 8) {
    $setname = "$($setname.SubString(0,5))$($setname.SubString($setname.length -3))"
  }
  ...
  if (Test-Path (Escape-Name $dstpath/$setname.rom)) {
    &"$TOOLS_ROOT/touch.exe" -r $mrafile "$dstpath/$setname.rom"
  } else {
    Write-Host -ForegroundColor red "  ERROR: `'$dstpath/$setname.rom`' not found"
  }
  if (Test-Path (Escape-Name $dstpath/$arcname.arc)) {
    if ($name -ne $arcname) { Move-Item (Escape-Name $dstpath/$arcname.arc) -Destination $dstpath/$name.arc -Force }
...
function process_mra {
  ...
  Write-Host "`r`n$($mrafile.Replace("$GIT_ROOT/",'')) ($name, $rbf, $zips):"

  # prepare target folder with .mra, .rbf .rom and .zip files
  if ((-not (Test-Path (Escape-Name $dstpath/$name.arc)) -Or (-not (Test-Path (Escape-Name $dstpath/$rbf.rbf))))) {
    ...
    if ($rbfpath -ne '') {
      ...
      if (Test-Path $srcrbf) {
        ...
        Write-Host -ForegroundColor red "  ERROR: `"$srcrbf`" not found"
      }
    }
...
function copy_jotego_arcade_cores {
  ...
  clone_or_update_git 'http://github.com/jotego/jtbin.git' $srcpath | Out-Null

  # ini file from jotego git
  #coppy "$srcpath/arc/mist.ini" "$dstroot/"

  # generate destination arcade folders from .mra and .core files
  copy_mra_arcade_cores "$srcpath/mra" "$srcpath/$fpga" $dstroot $jtlookup
  foreach ($dir in (Get-ChildItem "$srcpath/mra" -Directory -Recurse | Select-Object -ExpandProperty FullName | Sort | replace-slash)) {
...
function copy_gehstock_mist_cores {
  ...
  foreach ($rbf in (Get-ChildItem "$srcroot/*.rbf" -Recurse | Sort-Object -Property FullName | replace-slash)) {
    ...
    $dst = ($dir.Replace($srcroot,$dstroot) -ireplace '_MiST', '').Replace('/Arcade/','/Arcade/Gehstock/')
...
function copy_sorgelig_mist_cores {
  ...
  Write-Host "`r`n----------------------------------------------------------------------"
  Write-Host "copy Sorgelig Cores for `'mist`' to `'$dstroot`'"
  Write-Host "----------------------------------------------------------------------`r`n"
...
function copy_other_mist_cores {
  param ( [string]$dstroot ) # $1: target folder
...
function amstrad_roms       { param ( $1, $2 )
                              ...
                              download_url 'http://raw.githubusercontent.com/mist-devel/mist-binaries/master/cores/amstrad/ROMs/AST-Equinox.dsk' "$2/roms/" | Out-Null
...
function apple1_roms_alt    { param ( $1, $2 )
                              download_url 'http://raw.githubusercontent.com/mist-devel/mist-binaries/master/cores/apple1/BASIC.e000.prg' "$2/" | Out-Null
                              download_url 'http://raw.githubusercontent.com/mist-devel/mist-binaries/master/cores/apple1/DEMO40TH.0280.prg' "$2/" | Out-Null
                            }
function apple2e_roms       { param ( $1, $2 )
                              ...
                              download_url 'http://ia802800.us.archive.org/4/items/PitchDark/Pitch-Dark-20210331.zip' "$2/" | Out-Null
                              expand "$2/Pitch-Dark-20210331.zip" "$2/"
                            }
function apple2p_roms       { param ( $1, $2 )
                              download_url 'http://raw.githubusercontent.com/wsoltys/mist-cores/mockingboard/apple2fpga/apple_II.rom' "$2/" | Out-Null
                              download_url 'http://raw.githubusercontent.com/wsoltys/mist-cores/mockingboard/apple2fpga/bios.rom' "$2/" | Out-Null
                            }
function archimedes_roms    { param ( $1, $2 )
                              download_url 'http://raw.githubusercontent.com/MiSTer-devel/Archie_MiSTer/master/releases/riscos.rom' "$2/" | Out-Null
                              coppy "$2/riscos.rom" "$SD_ROOT/"
                              coppy "$1/SVGAIDE.RAM" "$SD_ROOT/svgaide.ram"
                              download_url 'http://raw.githubusercontent.com/mist-devel/mist-binaries/master/cores/archimedes/archie1.zip' "$2/" | Out-Null
                              expand "$2/archie1.zip" "$SD_ROOT/"
                              expand "$1/RiscDevIDE.zip" "$2/"
                            }
function atarist_roms       { param ( $1, $2 )
                              download_url 'http://raw.githubusercontent.com/mist-devel/mist-binaries/master/cores/mist/tos.img' "$SD_ROOT/" | Out-Null
                              download_url 'http://raw.githubusercontent.com/mist-devel/mist-binaries/master/cores/mist/system.fnt' "$SD_ROOT/" | Out-Null
                              download_url 'http://raw.githubusercontent.com/mist-devel/mist-binaries/master/cores/mist/disk_a.st' "$2/" | Out-Null
                            }
...
function bbc_roms           { param ( $1, $2 )
                              coppy "$1/bbc.rom" "$2/"
                              download_url 'http://raw.githubusercontent.com/ManuFerHi/SiDi-FPGA/master/Cores/Computer/BBC/BBC.vhd' "$2/" | Out-Null
...
function c64_roms           { param ( $1, $2 )
                              coppy "$1/c64.rom" "$2/"
                              download_url 'http://www.labaya-retromarketplace.it/sites/default/files/gianasister.prg' "$2/roms/" | Out-Null
                             #curl -O "$2/roms/SuperZaxxon.zip" -d 'id=727332&download=Télécharger' 'http://www.planetemu.net/php/roms/download.php'
                              ...
                              coppy "$2/roms/gianasister.prg" "$SD_ROOT/c64/" # roms are presented by core from /C64 folder
...
function menu_image         { param ( $1, $2 ) download_url 'http://raw.githubusercontent.com/mist-devel/mist-binaries/master/cores/menu/menu.rom' "$2/" | Out-Null }
function msx1_roms          { param ( $1, $2 ) expand "$1/MSX1_vhd.rar" "$2/" }
function msx2p_roms         { return; }
function nes_roms           { param ( $1, $2 )
                              download_url 'https://archive.org/download/mister-console-bios-pack_theypsilon/MiSTer_Console_BIOS_PACK.zip/NES.zip' "$2/" | Out-Null
                              expand "$2/NES.zip" "$2/"
...
function next186_roms       { param ( $1, $2 )
                              ...
                              download_url 'http://ia804501.us.archive.org/22/items/next-186.vhd/Next186.vhd.zip' "$2/hd/" | Out-Null
...
function ondra_roms         { param ( $1, $2 )
                              # https://github.com/PetrM1/OndraSPO186_MiST#loading-games-via-ondra-sd
                              # https://drive.google.com/file/d/1seHwftKzaBWHR4sSZVJLq7IKw-ZLafei
                            }
function oric_roms          { param ( $1, $2 ) if ($SYSTEM -eq 'mist') { coppy "$1/oric.rom" "$2/" } }
...
function pet2001_roms       { param ( $1, $2 ) download_url 'http://raw.githubusercontent.com/mist-devel/mist-binaries/master/cores/pet2001/pet2001.rom' "$2/" | Out-Null }
function plus_too_roms      { param ( $1, $2 ) download_url 'http://raw.githubusercontent.com/ManuFerHi/SiDi-FPGA/master/Cores/Computer/Plus_too/plus_too.rom' "$2/" | Out-Null
                              expand "$1/hdd_empty.zip" "$2/"
                            }
function ql_roms            { param ( $1, $2 )
                              download_url 'http://raw.githubusercontent.com/mist-devel/mist-binaries/master/cores/ql/QXL.WIN' "$2/" | Out-Null
                              download_url 'http://raw.githubusercontent.com/mist-devel/mist-binaries/master/cores/ql/QL-SD.zip' "$2/" | Out-Null
...
function vectrex_roms       { param ( $1, $2 )
                              download_url 'http://ia902801.us.archive.org/33/items/VectrexROMS/Vectrex_ROMS.zip' "$2/roms/" | Out-Null
...
function videopac_roms      { param ( $1, $2 )
                              download_url 'http://f.s3roms.download/romfiles/philips-videopac/l/loony-balloon-europe-usa.zip' "$2/roms/" | Out-Null
                              download_url 'http://f.s3roms.download/romfiles/philips-videopac/a/air-battle-europe-usa.zip' "$2/roms/" | Out-Null
                              expand "$2/roms/air-battle-europe-usa.zip" "$SD_ROOT/videopac/"
                              expand "$2/roms/loony-balloon-europe-usa.zip" "$SD_ROOT/videopac/"
                            }
function zx8x_roms          { param ( $1, $2 ) download_url 'http://raw.githubusercontent.com/ManuFerHi/SiDi-FPGA/master/Cores/Computer/ZX8X/zx8x.rom' "$2/" | Out-Null }
...
function bagman_roms        { param ( $1, $2 )
                              download_url 'http://raw.githubusercontent.com/Gehstock/Mist_FPGA/master/Arcade_MiST/Bagman Hardware/meta/Super Bagman.mra' "$env:TEMP/" | Out-Null
                              process_mra "$env:TEMP/Super Bagman.mra" "$2"
                              #Remove-Item "$env:TEMP/Super Bagman.mra" -Force
                            }
...
function show_usage {
  ...
  Write-Host "    If this option isn't specified, `'$SD_ROOT`' will be used by default."
  Write-Host " -s <mist|sidi>"
  Write-Host "    Set target system (mist or sidi)."
  Write-Host "    If this option isnt specified, `'$SYSTEM`' will be used by default."
  Write-Host " -h"
