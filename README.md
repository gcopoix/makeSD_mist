...
For testing/debugging specific cores, please refere some [test code](Linux/genSD.sh#L1298-L1317) left disabled in the scripts.
...
- **.mra parsing of ROM files** \
  ...
  Unfortunetly many ROMS, if fetched from their referred MAME version, don't match. I tried to find a best matching [set of download URLs](Linux/genSD.sh#L213-L253) incl. some [extra handling](Linux/genSD.sh#L254-L278), but for some ROM archives `mra` still complains about checksum mismatch or missing parts: \
  **ROMs not found**: \
...
- **Missing Jotego Cores** \
  Some .mra files in the Jotego repository refer missing .rbf files in the [mist](https://github.com/jotego/jtbin/tree/master/mist), [sidi](https://github.com/jotego/jtbin/tree/master/sidi) or [sidi128](https://github.com/jotego/jtbin/tree/master/sidi128) folders (->[issue](https://github.com/jotego/jtbin/issues/345)): \
   ...
   - `jtoutrun.rbf`, `jtkiwi.rbf`: \
     not enough ressources with MiST/SiDi according Jotego, although Somhi offers builds for **MiST** and **SiDi (fetched [here](Linux/genSD.sh#L507-L510) in the scripts).
- **MiSTer support** \
